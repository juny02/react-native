...
export default function Input() {
    ...
    const scrollViewRef= useRef<KeyboardAwareScrollView | null>(null) //그버그없는 그 스크롤메서드꺼내기위해 뷰의 ref 선언
    const scrollToInput = (reactNode: any) => {
        scrollViewRef.current?.scrollToFocusedInput(reactNode) //그함수 사용중 ~
    }
    const autoFocus = useCallback(
        ()=>(event: NativeSyntheticEvent<TextInputFocusEventData>) => {
            scrollToInput(findNodeHandle(event.target)) //그 위 함수로 autoFocus 만듬. 머 원래 이렇게 하나벼..
        }, [])
